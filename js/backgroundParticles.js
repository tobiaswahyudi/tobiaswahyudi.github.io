...
class BackgroundParticle {
  ...
  render(context, screenTop) {
    ...
    // context.fillStyle = `rgba(131,203,234,${this.opacity})`;
    context.fillStyle = `rgb(131,203,234)`;
...
const renderParticles = (ctx, width, height, scrollTop) => {
  ...
  for(var i = 0; i < particles.length; i++) {
    ...
    if(!particles[i].isOnScreen(scrollTop, width, height)) {
      particles[i] = randomParticle(0, screenWidth, screenHeight);
    }
  };
}